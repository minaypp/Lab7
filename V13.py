...
class TrainSystem:
    ...
    def process_alighting(self):
        ...
        for emergency in self.onboard_emergencies[:]:
            if emergency.destination_station == self.train_location:
                ...
                wait_time = emergency.arrival_time - emergency.request_time
                self.total_wait_time += wait_time
                self.total_passengers += 1
                print(f"Time {self.current_time}: Emergency alighted at station {self.train_location}, wait time {wait_time}")
        ...
        for passenger in self.onboard_passengers[:]:
            if passenger.destination_station == self.train_location:
                ...
                wait_time = passenger.arrival_time - passenger.request_time
                self.total_wait_time += wait_time
                self.total_passengers += 1
                print(f"Time {self.current_time}: Passenger alighted at station {self.train_location}, wait time {wait_time}")
    ...
    def run(self):
        ...
        if self.total_passengers > 0:
            average_wait_time = self.total_wait_time / self.total_passengers
